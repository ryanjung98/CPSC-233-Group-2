...
for (int i=0;i<8;i++){
   ...
   // loops through arraylist and prints board
	...
	public boolean scanBoard (String s){
		...
		i=Character.getNumericValue(x1);
		j=Character.getNumericValue(x2);
		l=Character.getNumericValue(y1);
		k=Character.getNumericValue(y2);
...
public char getPiece(int x, int y){
	char piece = chessBoard.get(x).get(y).getP();
	return piece;		
}
//getter
