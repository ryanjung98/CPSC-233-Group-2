	...
	//then if not sees if any one player can move so that the board is no longer in check. 	
		for (int it2=0;it2<super.chessBoard.size();it2++){
			for (int jt2=0 ;jt2<super.chessBoard.get(it2).size();jt2++){
				if (super.chessBoard.get(it2).get(jt2)!=null&&super.chessBoard.get(it2).get(jt2).getW()!=this.color&&super.chessBoard.get(it2).get(jt2).canMove(xpos,ypos)){
					for (int it3=0;it3<super.chessBoard.size();it3++){
						for (int jt3=0; jt3<super.chessBoard.get(it3).size();jt3++){
							if (super.chessBoard.get(it3).get(jt3)!=null&&super.chessBoard.get(it3).get(jt3).getW()==this.color&&super.chessBoard.get(it3).get(jt3).canMove(it2,jt2)&&super.isCheck(super.isGoingToBeInCheck(it3,jt3,it2,jt2))==false){
								move=this.toString(it3,jt3,it2,jt2);
								
								}
							else if (super.chessBoard.get(it3).get(jt3)!=null&&super.chessBoard.get(it3).get(jt3).getW()==this.color)
							{ for (int it4=0;it4<super.chessBoard.size();it4++){
								for (int jt4=0 ;jt4 < super.chessBoard.get(it4).size();jt4++){
									if (super.chessBoard.get(it3).get(jt3).canMove(it4,jt4)&&super.isCheck(super.isGoingToBeInCheck(it3,jt3,it4,jt4))==false){
										move=this.toString(it3,jt3,it4,jt4);
										}
	...
	for (int it1 =0; it1< super.chessBoard.size();it1++){
		for (int jt1=0;jt1<super.chessBoard.get(it1).size();jt1++){
			if (super.chessBoard.get(it1).get(jt1)!=null&&super.chessBoard.get(it1).get(jt1).getW()==this.color){
				for (int it2=0; it2<super.chessBoard.size();it2++){
					for (int jt2=0; jt2<super.chessBoard.get(it2).size();jt2++){
						if (super.chessBoard.get(it2).get(jt2)!=null&&super.chessBoard.get(it2).get(jt2).getW()!=this.color&&super.chessBoard.get(it1).get(jt1).canMove(it2,jt2)&&super.isCheck(isGoingToBeInCheck(it1,jt1,it2,jt2))==false){
							if (super.chessBoard.get(it2).get(jt2).getValue()>pieceValue){
