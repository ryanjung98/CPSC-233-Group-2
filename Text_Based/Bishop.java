	...
	if (x>this.getX()){
		...
		if (y>this.getY()&&(y-this.getY()==x-this.getX())){
			...
			for (int i=this.getX();i<x-1;i++){
				...
				if (Board.dummy.get(this.getX()+extra).get(this.getY()+extra)==null){
					n+=1;
		...
		if (n>=0){
		if (Board.dummy.get(x).get(y)!=null){
					//System.out.println("hi");
				if (Board.dummy.get(x).get(y).getW()!=this.getW()){
						n+=1;
						}
					}
		else if (Board.dummy.get(x).get(y)==null){
			n+=1;
			}}
